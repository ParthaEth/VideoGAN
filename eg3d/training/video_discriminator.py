...
class DiscriminatorBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        '''input: tensor with shape batch, color_ch, depth, height, width'''
        assert input.dim() == 5
...
class VideoDiscriminator(nn.Module):
    ...
    def __post_init__(self):
        ...
        self.blocks = nn.ModuleList([
            DiscriminatorBlock(32, 64, self.channels, spatial_ksize=self.spatial_ksize, temporal_ksize=1,
                               temporal_down=False, spatial_down=self.max_edge > 32, use_fp16=self.num_fp16_res > 0,
                               conv_clamp=self.conv_clamp),
