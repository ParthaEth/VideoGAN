...
from .blocks import conv2d, DownBlock, DownBlockPatch
...
class ProjectedDiscriminator(torch.nn.Module):
    ...
    def forward(self, x, c, update_emas=None, **kwargs):
        # x.shape = batch, 6, height, width
        logits = 0
        ...
        for bb_name, feat in self.feature_networks.items():
            ...
            mean = feat.normstats['mean']
            std = feat.normstats['std']
            if x_aug.shape[1] == 6:
                mean = np.concatenate((mean, mean))
                std = np.concatenate((std, std))

            x_n = Normalize(mean, std)(x_aug)
