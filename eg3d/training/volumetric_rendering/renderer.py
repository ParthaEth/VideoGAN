...
class SampleUsingMHA(torch.nn.Module):
    def __init__(self, num_plane_features):
        super().__init__()
        # self.projector = mh_projector.MHprojector(motion_feature_dim=num_plane_features//2,
        #                                           appearance_feature_dim=num_plane_features-num_plane_features//2,
        #                                           num_heads=4)
        # self.projector = mh_projector.TransformerProjector(proj_dim=num_plane_features, num_heads=8)
        self.projector = mh_projector.TinyMHprojector(proj_dim=num_plane_features, num_heads=4)

    def forward(self, z, plane_features, coordinates, recompute_full_vid_features, bypass_network=False):
        ...
        if bypass_network:
            ...
            output_features, attention_mask = self.projector(z, plane_features.squeeze(), coordinates,
...
class ImportanceRenderer(torch.nn.Module):
    ...
    def forward(self, z, planes, decoder, ray_origins, ray_directions, rendering_options):
    ...
    def run_model(self, z, planes, decoder, sample_coordinates, options, render_one_frame, recompute_full_vid_features,
                  bypass_network=False):
        sampled_features, attention_mask = self.projector(z, planes, sample_coordinates, recompute_full_vid_features,
...
class AxisAligndProjectionRenderer(ImportanceRenderer):
    ...
    def forward(self, z, planes, decoder, c, ray_directions, rendering_options):
        ...
        out, img_attn_mask = self.run_model(z, planes, decoder, sample_coordinates, rendering_options,
        ...
        out, vid_attn_mask = self.run_model(z, planes, decoder, video_coordinates, rendering_options,
