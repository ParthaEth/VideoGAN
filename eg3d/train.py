...
@click.option('--workers',      help='DataLoader worker processes', metavar='INT',              type=click.IntRange(min=0), default=8, show_default=True)
...
def main(**kwargs):
    ...
    c.data_loader_kwargs = dnnlib.EasyDict(pin_memory=False, prefetch_factor=None)
    ...
    c.training_set_kwargs.return_video = opts.return_video
    ...
    if opts.aug != 'noaug':
        c.augment_kwargs = dnnlib.EasyDict(class_name='training.augment.AugmentPipe',
                                           xflip=1, rotate90=0, xint=1, scale=1, rotate=0, aniso=1, xfrac=1,
