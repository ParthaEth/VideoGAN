...
def t_loop(rank, random_seed, num_gpus, batch_p_gpu, dataset_dir, out_dir, input_vid_res, tri_plane_res,
           motion_features, appearance_feat,
    ...
    training_set_iterator = iter(torch.utils.data.DataLoader(dataset=training_set, sampler=training_set_sampler,
                                                             batch_size=batch_p_gpu, pin_memory=False,
    ...
    while True:
        ...
        for param in mdl_params:
            ...
            param.grad = flat.reshape(param.shape)
        ...
        if rank == 0:
            ...
            pbar.update(1)
            # print(
            #     f'iteration: {batch_id} loss: {np.mean(losses[-10:]):0.6f} PSNR: {psnr_lr:0.2f}, PSNR_best:{best_psnr:0.2f}')
            # scheduler.step(np.mean(losses[-10:]))

            if batch_id % 200 == 199:
                # Save original and reconstructed video
                save_video(peep_vid.detach(), peep_vid_real, os.path.join(out_dir, f'{batch_id}'))

        if train_itr <= batch_id:
            print('Done')
            break
...
if __name__ == '__main__':
    ...
    c.dataset_dir = '/is/cluster/fast/pghosh/datasets/ffhq_X_celebv_hq_1_motions'
    c.out_dir = '/is/cluster/fast/pghosh/ouputs/video_gan_runs/single_vid_over_fitting'
    ...
    c.batch_p_gpu = 2
    assert c.batch_p_gpu > 1, 'because of squeeze function inside renderer a batch size 1 mess dimensionality up'
    c.num_gpus = 8
    c.random_seed = 0
    c.train_itr = 50000
